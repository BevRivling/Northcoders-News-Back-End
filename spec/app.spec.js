...
describe("/", () => {
  ...
  describe("/api/topics", () => {
    ...
    it("POST with a malformed body responds with a 400 and an explaination of the error", () =>
      request
        .post(url)
        .send({
          slag: "unique test slug with wrong key",
          description: "too many keys",
          description2: "way too many keys"
        })
        .expect(400)
        .then(({ body }) => {
          ...
          request[method](url)
            ...
            .then(({ body }) => {
              expect(body.msg).to.equal(
                "Bad Request: method not available for this endpoint"
              );
            })
        )
      );
    ...
    it("POST with a legitimate request but invalid entity responds with 422 and explaination of the error", () =>
      request
        ...
        .send({
          slug: "mitch",
          description: "slug - mitch already exists in the table"
        })
        .expect(422)
        .then(({ body }) => {
    ...
    describe("/api/topics/:topic/articles", () => {
      ...
      it("GET returns a 200 and all the articles under a particular topic through a valid topic_id", () =>
        request
          ...
          .then(({ body }) => {
            ...
            expect(body.msg[0]).to.have.all.keys([
              "title",
              "article_id",
              "topic",
              "author",
      ...
      it("GET applies valid queries on the results while ignoring the invalid ones", () =>
        Promise.all([
          request
            .get(`${urlTopicArticle}?limit=5`)
            .expect(200)
            .then(({ body }) => {
              expect(body.msg.length).to.equal(5);
            }),
          request
            .get(`${urlTopicArticle}?p=2`)
            .expect(200)
            .then(({ body }) => {
              expect(body.msg[0].title).to.equal("Moustache");
            }),
          request
            .get(`${urlTopicArticle}?sort_by=topic&sort_ascending=true`)
            .expect(200)
            .then(({ body }) => {
              expect(body.msg[0].title).to.equal(
                "They're not exactly dogs, are they?"
              );
            }),
          request
            .get(`${urlTopicArticle}?turkey=false&captain_birdseye=99`)
            .expect(200)
            .then(({ body }) => {
              expect(body.msg.length).to.equal(10);
            })
      ...
      it("GET and POST with incorrect parametric endpoints return a 404", () =>
        request
          .get(`${url}/bevan/articles`)
          .expect(404)
          .then(({ body }) => {
      ...
      it("All http methods except GET and POST return a 405 and an explaination of the error", () => {
        ...
        return Promise.all(
          invalidMethods.map(method =>
            request[method](urlTopicArticle)
              .expect(405)
              .then(({ body }) => {
                expect(body.msg).to.equal(
                  "Bad Request: method not available for this endpoint"
  ...
  describe("/api/articles", () => {
    ...
    it("GET applies valid queries on the results while ignoring the invalid ones", () =>
      Promise.all([
        ...
        request
          .get(`${url}?sort_by=topic&sort_ascending=true`)
          .expect(200)
          .then(({ body: { articles } }) => {
        ...
        request
          .get(`${url}?turkey=false&captain_birdseye=99`)
          .expect(200)
          .then(({ body: { articles } }) => {
    ...
    describe("/api/articles/:article_id", () => {
      it("GET returns a 200 and an article object when passed a valid article_id", () =>
        request
          .get(`${url}/1`)
          .expect(200)
          .then(({ body }) => {
      ...
      it("PATCH returns a 202 and increments the targeted article's vote property by the designated amount", () =>
        request
          .patch(`${url}/1`)
          .send({ inc_votes: 1 })
          .expect(202)
      ...
      it("DELETE removes the article, returns a 204 status and an empty object", () =>
        request
          .delete(`${url}/1`)
          .expect(204)
          .then(({ body }) => {
      ...
      it("GET, PATCH, and DELETE with incorrect parametric endpoints return a 404", () =>
        Promise.all([
          request
            .get(`${url}/99`)
            .expect(404)
    ...
    describe("/api/articles/:article_id/comments", () => {
      ...
      it("GET returns a 200 and an array of the comments associated with a valid article_id", () =>
        request
          .get(`${urlArticleComments}`)
          .expect(200)
          .then(({ body: { comments } }) => {
      ...
      it("GET applies valid queries on the results while ignoring the invalid ones", () =>
        Promise.all([
          request
            .get(`${urlArticleComments}?limit=3`)
            .expect(200)
            .then(({ body: { comments } }) => {
              expect(comments.length).to.equal(3);
            }),
          request
            .get(`${urlArticleComments}?p=2`)
            .expect(200)
            .then(({ body: { comments } }) => {
              expect(comments[0].comments_id).to.equal(10);
            }),
          request
            .get(`${urlArticleComments}?sort_by=topic&sort_ascending=true`)
            .expect(200)
            .then(({ body: { comments } }) => {
              expect(comments[0].comments_id).to.equal(1);
            }),
          request
            .get(`${urlArticleComments}?turkey=false&captain_birdseye=99`)
            .expect(200)
            .then(({ body: { comments } }) => {
              expect(comments).to.be.an("array");
              expect(comments.length).to.equal(10);
      ...
      it("POST returns a 201 and the posted comment", () =>
        request
          .post(`${urlArticleComments}`)
          .send({
      ...
      it("POST with incorrect parametric endpoints return a 404", () =>
        request
      ...
      it("GET with incorrect parametric endpoints return a 404", () =>
        request
  ...
  describe("/api/comments", () => {
    describe("/api/comments/:comment_id", () => {
      ...
      it("PATCH returns a 202 increments the targeted comment's vote property by the designated amount", () =>
        Promise.all([
          request
            .patch(`${url}/1`)
            .send({ inc_votes: 1 })
      ...
      it("DELETE removes the comment, returns a 204 status and an empty object", () =>
        request
          .delete(`${url}/1`)
          .expect(204)
          .then(({ body }) => {
  ...
  describe("/api/users", () => {
    const url = "/api/users";
    it("GET returns a 200 and an array of user objects", () =>
      request
        .get(url)
        .expect(200)
        .then(({ body: { users } }) => {
    ...
    describe("/api/users/:username", () => {
      it("GET returns a 200 and a user with the associated username", () =>
        request
          .get(`${url}/butter_bridge`)
          .expect(200)
          .then(({ body }) => {
            expect(body[0]).to.be.an("object");
    ...
    it("GET with an incorrect parametric endpoint return a 404", () =>
      request
        .get(`${url}/incorrect_username`)
        .expect(404)
        .then(({ body }) => {
          expect(body.msg).to.equal("Page not found");
        }));
